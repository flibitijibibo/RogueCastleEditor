...
namespace RogueCastleEditor
{
    ...
    public class GraphicsDeviceControl : HwndHost
    {
        ...
        public void SetCursor(Cursor cursor)
        {
            ...
            NativeMethods.SetClassLong(hWnd, -12, _cursorPtr);
        }
        ...
        void XnaWindowHost_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            if (graphicsService == null)
            {
                // In case the GraphicsDeviceControl is hidden behind something, like a separate tab.
                if (ActualWidth == 0 && ActualHeight == 0)
                {
                    //graphicsService = GraphicsDeviceService.AddRef(hWnd, (int)Width, (int)Height);
                    //Viewport viewport = new Viewport(0, 0, (int)Width, (int)Height);
                    graphicsService = GraphicsDeviceService.AddRef(hWnd, (int)lastWidth, (int)lastHeight);
                    Viewport viewport = new Viewport(0, 0, (int)lastWidth, (int)lastHeight);
                    graphicsService.GraphicsDevice.Viewport = viewport;
                }
                else
                {
                    graphicsService = GraphicsDeviceService.AddRef(hWnd, (int)ActualWidth, (int)ActualHeight);
                    Viewport viewport = new Viewport(0, 0, (int)ActualWidth, (int)ActualHeight);
        ...
        {
            NativeMethods.SetCursorPos(x - capturedMouseClientX, y - capturedMouseClientY);
            mouseState.Position = mouseState.PreviousPosition = new Point(capturedMouseClientX, capturedMouseClientY);
        }
